...
def test_sfn_ecs_sync_poll_waits_for_stopped_and_propagates_failure(monkeypatch):
    """The sync integration waits past active states, then propagates failure."""
    ...
    with pytest.raises(sfn_service._ExecutionError) as exc:
        sfn_service._poll_ecs_tasks("cluster", ["task-arn"])

    assert len(seen) == 3
    assert exc.value.error == "States.TaskFailed"
    assert json.loads(exc.value.cause)["Containers"][0]["ExitCode"] == 17
